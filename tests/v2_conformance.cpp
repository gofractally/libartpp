// artpp_conformance.cpp — STL-grade conformance and edge-case suite for artpp::v2::map.
//
...
static void test_pool_bulk_move()
{
   ...
   artpp::v2::line_pool pa(size_t(1) << 30), pb(size_t(1) << 30);
   Tree           a{A{&pa}}, b{A{&pb}};
   std::map<std::string, uint64_t> ref;
...
static void test_fused_prefix()
{
   ...
   {
      artpp::v2::map<uint64_t, uint64_t> t;
   ...
   {
      artpp::v2::map<std::string_view, uint32_t> t;
...
{
   using OwnK  = artpp::v2::map<std::string, uint64_t>;
   using ViewK = artpp::v2::map<std::string_view, uint64_t>;
   ...
   using VecK = artpp::v2::map<std::vector<uint8_t>, uint32_t>;
   static_assert(
