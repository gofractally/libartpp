...
{
   int rc = 0;
   rc |= run<artpp::v2::map<std::string_view, uint64_t>>("mode::none   ", 42, 30);
   rc |= run<artpp::v2::map<std::string_view, uint64_t, artpp::v2::mode::buckets>>("mode::buckets", 43, 30);
   rc |= run<artpp::v2::map<std::string_view, uint64_t,
                          artpp::v2::mode(unsigned(artpp::v2::mode::adaptive) | unsigned(artpp::v2::mode::dense_tiers))>>(
       "adaptive|dt  ", 44, 30);
   rc |= run<artpp::v2::map<std::string_view, uint32_t>>("u32 inline   ", 45, 30);
   return rc;
}
